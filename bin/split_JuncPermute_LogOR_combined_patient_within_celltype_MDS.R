...
for (file in list.files(path.to.genotype)){
  ...
  genotype = geno[,c("Genotype_1UMI_mutUMIFrac0.2", "Cell.Assignment")] ## change Cell.Assignment column for MDS patients
  ...
  genotype = genotype[which(genotype$Genotype_1UMI_mutUMIFrac0.2 != "AMB"),]
...
for (patient in patient.names){
  wt = rownames(genotype.list[[patient]])[genotype.list[[patient]]$Genotype_1UMI_mutUMIFrac0.2 == "WT"]
  mut = rownames(genotype.list[[patient]])[genotype.list[[patient]]$Genotype_1UMI_mutUMIFrac0.2 == "MUT"]
...
for(x in 0:nperm){
  ...
  for (patient in patient.names) {
    ...
    for(type in cell.types.list[[patient]]){
      geno_vec = as.character(genotype.list[[patient]][which(genotype.list[[patient]]$Cell.Assignment == type),"Genotype_1UMI_mutUMIFrac0.2"])
