...
#SBATCH --job-name=MDS.P5.2_celltype_bams
#SBATCH --mem=100g
...
do 
cut -d " " -f 1,7,9 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_3$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,7,9 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_3$//' > "$type"_mut_barcodes.txt 
