...
#SBATCH --job-name=MDS.P5.1.celltype_bams
#SBATCH --mem=100g
...
#sorted_unique_ids=($(cut -d" " -f9 $metadata | sort -u | tr '\n' ' '))
#sorted_unique_ids=("NA" "PreB" "Tcells")
sorted_unique_ids=("Mat_Mono")

for type in "${sorted_unique_ids[@]}"
do 
cut -d " " -f 1,7,9 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,7,9 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_mut_barcodes.txt 
