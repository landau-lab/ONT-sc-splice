...
do 
cut -d " " -f 1,5,8 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_1$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,5,8 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_1$//' > "$type"_mut_barcodes.txt 
