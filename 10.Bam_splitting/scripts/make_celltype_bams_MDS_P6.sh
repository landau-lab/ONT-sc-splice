...
#SBATCH --job-name=MDS_P6_celltype_bams
#SBATCH --mem=100g
...
do 
cut -d " " -f 1,7,9 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_4$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,7,9 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_4$//' > "$type"_mut_barcodes.txt 
