...
#SBATCH --job-name=MDS_P3_celltype_bams
#SBATCH --mem=100g
...
do 
cut -d " " -f 1,5,7 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_3$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,5,7 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_3$//' > "$type"_mut_barcodes.txt 
