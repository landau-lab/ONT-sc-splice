...
do 
cut -d " " -f 1,5,8 $genotype | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,5,8 $genotype | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_mut_barcodes.txt 
