...
do 
cut -d " " -f 1,8,10 $genotype | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_1$//' > "$type"_wt_barcodes.txt 
cut -d " " -f 1,8,10 $genotype | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_1$//' > "$type"_mut_barcodes.txt 
