...
#SBATCH --job-name=MDS_P5_1_2UMI_merge
#SBATCH --mem=100g
...
do
cut -d " " -f 1,8,9 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_wt_barcodes_2UMI.txt
cut -d " " -f 1,8,9 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_mut_barcodes_2UMI.txt
