...
sorted_unique_ids=("HSPC" "IMP" "EP" "MEP" "NP" "MkP" "MonoDC" "Mat_Mono" "DC" "Bcells" "NA" "PreB" "Tcells")

for type in "${sorted_unique_ids[@]}"
do
cut -d " " -f 1,8,9 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_4$//' > "$type"_wt_barcodes_2UMI.txt
cut -d " " -f 1,8,9 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_4$//' > "$type"_mut_barcodes_2UMI.txt
