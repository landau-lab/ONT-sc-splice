...
#SBATCH --job-name=CH305_2UMI_merge
#SBATCH --mem=100g
...
sorted_unique_ids=("HSPC" "IMP" "EP" "MEP" "NP" "MkP")

for type in "${sorted_unique_ids[@]}"
do
cut -d " " -f 1,6,8 $metadata | grep "WT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_wt_barcodes_2UMI.txt
cut -d " " -f 1,6,8 $metadata | grep "MUT $type$" | cut -d " " -f 1 | sed 's/\_2$//' > "$type"_mut_barcodes_2UMI.txt
