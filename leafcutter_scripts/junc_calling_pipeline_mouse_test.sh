...
#SBATCH --mem=200g
#SBATCH --output=/gpfs/commons/groups/landau_lab/lkluegel/stdout_%j.log
#SBATCH --error=/gpfs/commons/groups/landau_lab/lkluegel/stderror_%j.log
...
mkdir "$run_files"/annotation_reference_mouse

#Format: path/to/leafcutter/leafviz/gtf2leafcutter.pl -o path/to/output_files/prefix path/to/reference_gtf

/gpfs/commons/home/gmullokandov/software/leafcutter/leafviz/gtf2leafcutter.pl -o "$run_files"/annotation_reference_mouse/leafviz /gpfs/commons/groups/landau_lab/genomes/Mouse/refdata-gex-mm10-2020-A/genes/genes.gtf

annotation_code=""$run_files"/annotation_reference_mouse/leafviz"
...
scp "$run_files"/annotation_reference_mouse/leafviz_all_introns.bed.gz "$patient_run"_output/
...
python "$run_files"/intron_bed_cleaner.py "$output_location"/leafcutter_outputs/"$patient_run"_output/leafviz_all_introns.bed "$output_location"/leafcutter_outputs/"$patient_run"_output/leafviz_all_introns_cleaned.bed

#Format:python splice_annotator_got.py <path to datafiles> <input datafile> <junction tag .bed file> <output main file name>
python "$run_files"/new_annotator_with_skipping.py "$output_location"/leafcutter_outputs/"$patient_run"_output "$patient_run"_all.introns.info.txt "$output_location"/leafcutter_outputs/"$patient_run"_output/leafviz_all_introns.bed "$output_location"/leafcutter_outputs/"$patient_run"_output/leafviz_all_introns_cleaned.bed ONT "$patient_run"_all.introns.info.w.primary.annotations.txt
