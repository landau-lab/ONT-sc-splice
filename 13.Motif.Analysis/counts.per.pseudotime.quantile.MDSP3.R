...
##Using the Object grab the c("HSPC","HSPC_EP","IMP","IMP_MEP","EP_1","EP_2", "MkP") cells, rank by pseudotime and sperate into quantiles.

##----------------- Setting important variables   -----------------##

seurat.object.path <- "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/4.Integrated_seurat_objs/mds.integrated.Rdata"

junction.meta.file.path <- "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/7.Permutation_junction_output/1.ind_patient_logOR/100k_perm/MDS_P3/logOR_within_cell_type_ALT_3P_Junctions_with_threshold_info.txt"

junction.count.file.path <- "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/9.Splice_pipeline_example_run/MDS_P3/p3_biotin/ONT_pipeline/output_files/leafcutter_outputs/MDS_P3_allreads_output/MDS_P3_allreads_perind_numbers.counts.txt"

genotyping.table.file.path <- "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/3.Genotyping_info/mds_p3.genotype.info.with.cell.type.txt"
...
cluster.column = "Cell.Assignment" #to take from genotyping table
cluster.types = c("HSPC","HSPC_EP","IMP","IMP_MEP","EP_1","EP_2", "MkP")

##----------------- Load in Seurat Object   -----------------##

print("Loading in seurat object")
load(seurat.object.path) #samples.integrated
mds.samples.integrated <- mds.samples.integrated_nomature
...
m <- m[,-1]
colnames(m)[1:5]
colnames(m) <- paste0(colnames(m),"_3")
colnames(m)[1:5]
...
write.table(junction.meta.sub, file="/gpfs/commons/groups/landau_lab/SF3B1_splice_project/13.Motif.Analysis/junction.matricies/MDSP3.E662D.alt3p.junction.info.w.quantile.and.ct.counts.mutcells.only.full.txt", quote = T,row.names = TRUE, na = "NA",sep=",")

junction.meta.sub.final <- junction.meta.sub[,c("pvalue","intron_junction","five_prime_ID","gene","Final_Verdict","fivep_distance","threep_distance", "Q1.mut.junc.cov","Q2.mut.junc.cov","Q3.mut.junc.cov","Q4.mut.junc.cov")]


colnames(junction.meta.sub.final) <- c("pvalue","intron.junction","clusterID","gene","final.junction.verdict","fivep.distance","threep.distance","Q1.mut.junc.cov","Q2.mut.junc.cov","Q3.mut.junc.cov","Q4.mut.junc.cov")

write.table(junction.meta.sub.final, file=paste0("/gpfs/commons/groups/landau_lab/SF3B1_splice_project/13.Motif.Analysis/junction.matricies/MDSP3.E662D.alt3p.junction.info.w.quantile.and.ct.counts.mutcells.only.txt"), quote = F)
