...
junction.meta.file.path <- "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/13.Motif.Analysis/junction.matricies/MDSP3.E662D.alt3p.junction.info.w.quantile.and.ct.counts.mutcells.only.txt"
...
shared.cells <- intersect(cells.to.use, rownames(mds.samples.integrated@meta.data))
length(shared.cells)
length(cells.to.use)
cell.meta = as.data.frame(mds.samples.integrated@meta.data[shared.cells,])
cell.meta[1:5,1:5]
...
write.table(junction.meta.sub, file="/gpfs/commons/groups/landau_lab/SF3B1_splice_project/13.Motif.Analysis/junction.matricies/MDSP3.E662D.alt3p.junction.info.w.quantile.and.ct.and.bulk.counts.mutcells.only.txt", quote = F)
