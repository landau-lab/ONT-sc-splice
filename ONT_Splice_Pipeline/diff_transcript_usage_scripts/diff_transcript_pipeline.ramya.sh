...
permute_jobids=()
for i in {1..1000}; do
permute_jobids+=($(sbatch --job-name="$sample_name" "$run_files"/bin/run_split_perm_5p_3p_ramya.sh "$workdir"/diff_transcript_output/split_cluster_files/split_"$i" $genotype $nperm $pattern "$workdir"/diff_transcript_output/split_cluster_output output_"$i" "$run_files"/bin))
done 
