...
for(cluster in three.prime.clusters){
  ...
  for (junc in subset$intron_junction){
    three.obs.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"obs.mut"]/subset[which(subset$intron_junction == junc),"mut.remain"]) + 1e-5)/((subset[which(subset$intron_junction == junc),"obs.wt"]/subset[which(subset$intron_junction == junc),"wt.remain"])+1e-5))
  }
}
...
for (cluster in five.prime.clusters){
  ...
  subset$wt.remain = subset$wt.all - subset$obs.wt
  for (junc in subset$intron_junction){
    five.obs.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"obs.mut"]/subset[which(subset$intron_junction == junc),"mut.remain"]) + 1e-5)/((subset[which(subset$intron_junction == junc),"obs.wt"]/subset[which(subset$intron_junction == junc),"wt.remain"])+1e-5))
  }
}
...
for(x in 0:nperm){
  ...
  for (patient in patient.names) {
    ...
    geno_vec = as.character(genotype.list[[patient]][,"Genotype_1UMI"])
  ...
  for(clust in three.prime.clusters){
    ...
    for (junc in subset$intron_junction){
      three.shf.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"shf.mut"]/subset[which(subset$intron_junction == junc),"mut.remain"])+1e-5)/((subset[which(subset$intron_junction == junc),"shf.wt"]/subset[which(subset$intron_junction == junc),"wt.remain"])+1e-5))
    }
  }
  ...
  for(clust in five.prime.clusters){
    ...
    for (junc in subset$intron_junction){
      five.shf.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"shf.mut"]/subset[which(subset$intron_junction == junc),"mut.remain"])+1e-5)/((subset[which(subset$intron_junction == junc),"shf.wt"]/subset[which(subset$intron_junction == junc),"wt.remain"])+1e-5))
    }
  }
