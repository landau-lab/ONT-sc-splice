...
data.comb.list = list()
for (patient in patient.names){
  wt = rownames(genotype.list[[patient]])[genotype.list[[patient]] == "WT" & genotype.list[[patient]]$Cell.Assignment_2 == celltype]
  mut = rownames(genotype.list[[patient]])[genotype.list[[patient]] == "MUT" & genotype.list[[patient]]$Cell.Assignment_2 == celltype]
...
split_clusters_three = chunk(clust.three, 100)
split_clusters_five = chunk(clust.five, 100)
