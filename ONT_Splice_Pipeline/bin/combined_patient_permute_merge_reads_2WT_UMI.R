...
# Test data
# path.to.genotype = "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/9.Splice_pipeline_example_run/MDS_P5_P6_2WT/2.Genotype_info"
# patient.names = c("MDS_P5_1", "MDS_P5_2", "MDS_P6")
# pattern = c("_1", "_2", "_3")
...
for (file in list.files(path.to.genotype)){
  ...
  genotype = geno %>% select(Final_Genotype, Cell.Assignment_2)
  ...
  genotype = genotype[which(genotype$Final_Genotype != "AMB"),]
...
for(x in 0:nperm){
  ...
  for (patient in patient.names) {
    ...
    for(type in cell.types.list[[patient]]){
      geno_vec = as.character(genotype.list[[patient]][which(genotype.list[[patient]]$Cell.Assignment_2 == type),"Final_Genotype"])
