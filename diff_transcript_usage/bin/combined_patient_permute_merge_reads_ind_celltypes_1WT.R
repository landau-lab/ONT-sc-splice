...
# pattern = c("_1", "_2")
# path.to.split = "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/9.Splice_pipeline_example_run/CH_combined/diff_transcript_combined_merge_counts_ind_celltypes_1WT/EP/split_cluster_files/split_31"
...
for (file in list.files(path.to.genotype)){
  ...
  geno[which(geno$num.wt.calls_10x >= 1 & geno$num.mut.calls_10x ==0), "Final_Genotype"] = "WT"
...
for(cluster in three.prime.clusters){
  ...
  for (junc in subset$intron_junction){
    three.obs.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"obs.mut"] + 1e-5)/(subset[which(subset$intron_junction == junc),"mut.remain"] + 1e-5))/(((subset[which(subset$intron_junction == junc),"obs.wt"]+1e-5)/(subset[which(subset$intron_junction == junc),"wt.remain"]+1e-5))))
  }
}
...
for (cluster in five.prime.clusters){
  ...
  subset$wt.remain = subset$wt.all - subset$obs.wt
  for (junc in subset$intron_junction){
    five.obs.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"obs.mut"] + 1e-5)/(subset[which(subset$intron_junction == junc),"mut.remain"] + 1e-5))/(((subset[which(subset$intron_junction == junc),"obs.wt"]+1e-5)/(subset[which(subset$intron_junction == junc),"wt.remain"]+1e-5))))
  }
}
...
three.patient.output = data.frame(three.obs.logOR.ratio = three.obs.ratio.num, intron_junction = names(three.obs.ratio))
three.patient.output = left_join(three.patient.output, three.data.comb, by = "intron_junction")
#rownames(three.patient.output) = three.patient.output$alt_three_prime_intron_junction

five.patient.output = data.frame(five.obs.logOR.ratio = five.obs.ratio.num, intron_junction = names(five.obs.ratio))
five.patient.output = left_join(five.patient.output, five.data.comb, by = "intron_junction")
...
for(x in 0:nperm){
  ...
  for (patient in patient.names) {
    ...
    geno_vec = as.character(genotype.list[[patient]][,"Final_Genotype"])
    names(geno_vec) = rownames(genotype.list[[patient]])
    orig.names = names(geno_vec)
    shf.genotype = sample(geno_vec, size = length(geno_vec), replace = F)
  ...
  for(clust in three.prime.clusters){
    ...
    for (junc in subset$intron_junction){
      three.shf.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"shf.mut"] + 1e-5)/(subset[which(subset$intron_junction == junc),"mut.remain"] + 1e-5))/(((subset[which(subset$intron_junction == junc),"shf.wt"]+1e-5)/(subset[which(subset$intron_junction == junc),"wt.remain"]+1e-5))))
    }
  }
  ...
  for(clust in five.prime.clusters){
    ...
    for (junc in subset$intron_junction){
      five.shf.ratio[junc] = log(((subset[which(subset$intron_junction == junc),"shf.mut"] + 1e-5)/(subset[which(subset$intron_junction == junc),"mut.remain"] + 1e-5))/(((subset[which(subset$intron_junction == junc),"shf.wt"]+1e-5)/(subset[which(subset$intron_junction == junc),"wt.remain"]+1e-5))))
    }
  }
