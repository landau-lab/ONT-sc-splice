...
cluster.column = "Cell.Assignment_2"
min.num.umis <- 1
cluster.types = c("HSPC","IMP","MEP","EP")
patient.list = c("CH_305")
...
for (ct in cluster.types){
  print(ct)
  ct.mut.cells <- intersect(rownames(cell.meta.sub[which(cell.meta.sub$CellType == ct ),]),mut.cells)
  ...
  ct.wt.cells <- intersect(rownames(cell.meta.sub[which(cell.meta.sub$CellType == ct ),]),wt.cells)
...
write.table(junction.meta.sub, file="/gpfs/commons/groups/landau_lab/SF3B1_splice_project/16.Pseudotime.Window/BAX/CH.alt3p.junction.info.w.quantile.and.ct.counts.minUMI.1.CH305.txt", quote = T,row.names = TRUE, na = "NA",sep=",")
