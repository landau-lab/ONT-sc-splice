...
## test data
path.to.three.data = "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/7.Permutation_junction_output/3.combined_patients/MDS_P1_P2_P3/all_celltypes/combined_patient_logOR_within_cell_type_ALT_3P_Junctions.txt"
path.to.five.data = "/gpfs/commons/groups/landau_lab/SF3B1_splice_project/7.Permutation_junction_output/3.combined_patients/MDS_P1_P2_P3/all_celltypes/combined_patient_logOR_within_cell_type_ALT_5P_Junctions.txt"
...
##calculate weighted dPSI 

#calculate mean PSI
#patient.names = c("MDS_P1", "MDS_P2", "MDS_P3")
three.mat.names = lapply(patient.names, function(x) paste(x,"dPSI", sep = "_"))
three.obs.mat = three.data[,unlist(three.mat.names)]

five.mat.names = lapply(patient.names, function(x) paste(x,"dPSI", sep = "_"))
five.obs.mat = five.data[,unlist(five.mat.names)]

## weight by total junction coverage
three.total.junction.coverage = three.data[,grep("total.reads.per.junction", colnames(three.data))]
five.total.junction.coverage = five.data[,grep("total.reads.per.junction", colnames(five.data))]
...
i = 1
three.weight.dPSI = list()
while (i <= dim(three.weights)[1]){
  three.weight.dPSI[[i]] = rowWeightedMeans(as.matrix(three.obs.mat[i,]), as.numeric(three.weights[i,]))
  i = i +1
}
three.weight.dPSI = as.numeric(three.weight.dPSI)
names(three.weight.dPSI) = three.data$intron_junction

i = 1
five.weight.dPSI = list()
while (i <= dim(five.obs.mat)[1]){
  five.weight.dPSI[[i]] = rowWeightedMeans(as.matrix(five.obs.mat[i,]), as.numeric(five.weights[i,]))
  i = i +1
}
five.weight.dPSI = as.numeric(five.weight.dPSI)
names(five.weight.dPSI) = five.data$intron_junction

three.data$weight.dPSI = three.weight.dPSI
five.data$weight.dPSI = five.weight.dPSI
...
three.data = three.data %>% mutate(dPSI_threshold_0 = ifelse(weight.dPSI > 0, "yes", "no"),
                                   dPSI_threshold_5 = ifelse(weight.dPSI >= 5, "yes", "no"),
                                   pvalue_threshold = ifelse(pvalue < 0.05, "yes", "no"))
five.data = five.data %>% mutate(dPSI_threshold_0 = ifelse(weight.dPSI > 0, "yes", "no"),
                                 dPSI_threshold_5 = ifelse(weight.dPSI >= 5, "yes", "no"),
                                 pvalue_threshold = ifelse(pvalue < 0.05, "yes", "no"))
...
three.data[which(three.data$pvalue < 0.05 & three.data$weight.dPSI > 0),]$dPSI_0_pvalue_threshold = "yes"
three.data[which(three.data$pvalue < 0.05 & three.data$weight.dPSI >= 5),]$dPSI_5_pvalue_threshold = "yes"
five.data[which(five.data$pvalue < 0.05 & five.data$weight.dPSI > 0),]$dPSI_0_pvalue_threshold = "yes"
five.data[which(five.data$pvalue < 0.05 & five.data$weight.dPSI >= 5),]$dPSI_5_pvalue_threshold = "yes"
...
write.table(three.data, "CH_combined_logOR_ALT_3P_Junctions_with_threshold_info.txt")
write.table(five.data, "CH_combined_logOR_ALT_5P_Junctions_with_threshold_info.txt")
write.table(three.data.cryptic, "CH_combined_logOR_ONLY_CRYPTIC_3P_Junctions_with_threshold_info.txt")
write.table(five.data.cryptic, "CH_combined_logOR_ONLY_CRYPTIC_5P_Junctions_with_threshold_info.txt")